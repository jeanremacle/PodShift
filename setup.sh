...
# Create activation script (mode-aware)
...
EOF
    ...
    log_success "Created mode-aware activation script: $activate_script"
    
    # Provide mode-specific activation instructions
    if [[ "$PYTHON_MANAGER" == "pyenv" && "$DEPENDENCY_TOOL" == "uv" ]]; then
        log_info "Modern uv setup detected. Use 'uv run' commands or 'source ./activate.sh'"
        echo ""
        echo "Examples:"
        echo "  uv run python scripts/discovery/docker_inventory.py --help"
        echo "  source ./activate.sh  # For detailed uv usage instructions"
    else
        log_info "Traditional setup detected. To activate: source ./activate.sh"
    fi
}
